...
static void my_application_activate(GApplication *application) {
  ...
  const gboolean use_header_bar = [window]() -> gboolean {
    if (g_file_test(g_build_filename(g_get_user_data_dir(),
                                     "com.PiliMax.android", "use_ssd", NULL),
                    G_FILE_TEST_EXISTS))
      return FALSE;
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "pilimax");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "pilimax");
  }

  gtk_window_set_default_size(window, 1280, 720);

  g_autoptr(FlDartProject) project = fl_dart_project_new();
  // Keep Linux rendering on the stable GTK path until Impeller is reliable.
  ...
  // Wrap the Flutter view in a GtkOverlay so native child widgets (the GTK
  // WebKitWebView used by LinuxWebviewPlugin) can be stacked on top of it.
